...
The three base images are pinned to verified `linux/arm64` manifest digests,
and the Debezium archive is SHA-256 checked before extraction. If the selected
Strimzi operator does not support Kafka `4.0.0`, update the Strimzi/Kafka
versions and digest together; do not silently fall back to an amd64 image.
