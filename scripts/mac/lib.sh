...
readonly KIND_ARGO_MANIFESTS_DIR="${REPO_ROOT}/gitops/environments/kind/manifests/argocd"
...
activate_isolated_kind_kubeconfig() {
  ...
  kubectl --kubeconfig "${kubeconfig_path}" --context "${EXPECTED_CONTEXT}" \
    get nodes --request-timeout=10s >/dev/null ||
    die "Isolated context '${EXPECTED_CONTEXT}' is not reachable."
}

new_isolated_kubeconfig() {
  mktemp "${TMPDIR:-/tmp}/kind-local-kubeconfig.XXXXXX"
}
