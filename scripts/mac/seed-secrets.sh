...
seed_file_secret "polaris-key-pair" \
    "${polaris_public_key_file}" \
    "${polaris_private_key_file}"
...
seed_literal_secret "polaris-root-password" \
    "polaris-root-password" "${polaris_root_password}"
...
seed_literal_secret "warpstream-agent-api-key" \
    "agent_key" "${warpstream_agent_key}"
...
printf 'Local source secrets are seeded without displaying their values.\n'
