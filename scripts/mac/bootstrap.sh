...
Previews and then applies the dedicated kind-local infrastructure Pulumi stack
to install Argo CD. --yes is mandatory. The active context must be exactly
kind-kind, and Pulumi receives an isolated kubeconfig containing no rollback or
legacy context.
EOF
}
...
# apply. The apply phase repeats the target-cluster guard and uses a new
