...
project="all"
argocd_stack="mac-local"
applications_stack="mac-local"
...
Options:
  ...
  --argocd-stack NAME              (default: mac-local)
  --applications-stack NAME
  --yes                            Required: allow manifest files to be rewritten
  -h, --help

The script requires the active context to be exactly kind-dataplatform-mac.
...
clean_mac_argocd_manifests() {
  ...
  [[ "${resolved_path}" == "${MAC_ARGO_MANIFESTS_DIR}" ]] ||
