...
1. Copy `mac.env.example` to `kind.env`. An existing `mac.env` is still accepted.
...
4. Restrict the input files before using them:
   ...
   ```sh
   chmod 600 .local-secrets/kind.env .local-secrets/*.pem
   ```

5. After the `lakehouse-secrets` namespace and External Secrets resources have been
...
The seed script refuses to run outside `kind-kind`, validates local
file permissions, and never prints secret values. It creates or updates only
the source Secrets in the `lakehouse-secrets` namespace. Do not commit `kind.env`,
`mac.env`, PEM files, generated Secret YAML, or decoded Secret data.
